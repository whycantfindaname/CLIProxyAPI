...
| `.agents/`, `.claude/`, `.codex/`, `.trellis/`, `.zcode/`, `.gitattributes` | Preserve the branch-local Trellis workflow, platform adapters, hooks, agent definitions, Skills, specs, and workspace metadata used by Codex, Claude Code, ZCode, and the shared Agent Skills surface. | `python3 ./.trellis/scripts/get_context.py --mode packages`; `git diff --name-status origin/main...lwj_dev -- .agents .claude .codex .trellis .zcode .gitattributes` |
...
| `internal/pluginhost/host.go`, `internal/runtime/executor/claude_thinking_replay_test.go`, `internal/runtime/executor/codex_stream_bootstrap_buffering_test.go` | Keep the upstream-integrated Go source formatted with `gofmt`; these are formatting-only merge repairs with no intended behavior change. | `gofmt -l .`; `go test ./...` |
